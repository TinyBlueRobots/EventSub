...
namespace EventSub
{
    class SqlServerClient : ISqlClient
    {
        ...
        public async Task CreateSubscribersTable()
        {
            ...
            {
                await connection.ExecuteAsync("IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='Subscribers') CREATE TABLE Subscribers (Name VARCHAR(128), Subscriber TEXT, CONSTRAINT PK_Subscribers PRIMARY KEY (Name))");
            }
        }
        ...
        public async Task CreateSubscriber(Subscriber subscriber)
        {
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                await connection.ExecuteAsync($"IF NOT EXISTS (SELECT * FROM Subscribers WHERE Name='{subscriber.Name}') INSERT INTO Subscribers (Name, Subscriber) VALUES ('{subscriber.Name}','{json}')");
            }
        }
